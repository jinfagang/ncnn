...
static void draw_objects(const cv::Mat &bgr, const cv::Mat &fgr,
                         const cv::Mat &pha) {
    cv::Mat fgr8U;
    ...
    for (int i = 0; i < pha8U.rows; i++) {
        for (int j = 0; j < pha8U.cols; j++) {
            ...
            comp.at<cv::Vec3b>(i, j)[0] =
                fgr8U.at<cv::Vec3b>(i, j)[0] * alpha + (1 - alpha) * 155;
            comp.at<cv::Vec3b>(i, j)[1] =
                fgr8U.at<cv::Vec3b>(i, j)[1] * alpha + (1 - alpha) * 255;
            comp.at<cv::Vec3b>(i, j)[2] =
                fgr8U.at<cv::Vec3b>(i, j)[2] * alpha + (1 - alpha) * 120;
        }
    }
...
static int detect_rvm(const cv::Mat &bgr, cv::Mat &pha, cv::Mat &fgr) {
    ...
    // https://github.com/PeterL1n/RobustVideoMatting ncnn model
    // https://pan.baidu.com/s/11iEY2RGfzWFtce8ue7T3JQ password: d9t6
    if (net.load_param("rvm_512.param")) exit(-1);
    if (net.load_model("rvm_512.bin")) exit(-1);
    ...
    ncnn::Mat ncnn_in2 = ncnn::Mat::from_pixels_resize(
                             bgr.data, ncnn::Mat::PIXEL_BGR2RGB, bgr.cols, bgr.rows, target_width,
                             target_height);
    ncnn::Mat ncnn_in1 = ncnn::Mat::from_pixels_resize(
                             bgr.data, ncnn::Mat::PIXEL_BGR2RGB, bgr.cols, bgr.rows,
                             target_width * downsample_ratio, target_height * downsample_ratio);
    ...
    for (int i = 0; i < fgr_.h; i++) {
        for (int j = 0; j < fgr_.w; j++) {
            cv_fgr.at<cv::Vec3f>(i, j)[2] =
                fgr_data[0 * fgr_.h * fgr_.w + i * fgr_.w + j];
            cv_fgr.at<cv::Vec3f>(i, j)[1] =
                fgr_data[1 * fgr_.h * fgr_.w + i * fgr_.w + j];
            cv_fgr.at<cv::Vec3f>(i, j)[0] =
                fgr_data[2 * fgr_.h * fgr_.w + i * fgr_.w + j];
        }
    }
